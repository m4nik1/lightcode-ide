...
npm run web      # the Novus web app (Vite)
...
- Start the server before using `npm run web`. The Electron app starts its own
  embedded server. The web endpoint can be overridden with
  `VITE_NOVUS_SERVER_URL` and defaults to `http://127.0.0.1:2024`.
